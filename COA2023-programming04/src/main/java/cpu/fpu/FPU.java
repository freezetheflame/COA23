...
//        return NormalTreatment2(temp, exp, sign, a.sig.length());
        ...
        int exp = a.exp -b.exp + 127;
        if(a.exp==255){
            return "" + sign+"11111111"+String.join("",Collections.nCopies(32,"0"));
        }
        if(b.exp==255){
            return "" + sign+"00000000"+String.join("",Collections.nCopies(32,"0"));
        }
        String temp = unsignedDiv(a.sig,b.sig,a.sig.length());
        return NormalTreatment2(temp,exp,sign,a.sig.length());
...
//        }
//        String expStr = String.format("%8s", Integer.toBinaryString(exp)).replaceAll(" ", "0");
//        String expstr = Transformer.intToBinary(Integer.toString(exp)).substring(32-8);
//        return round(sign, expstr, sig);
        while(sig.charAt(0)!='1'&&exp>0){
            exp--;
            sig=sig.substring(1)+"0";
        }
        while(!sig.equals(String.join("",Collections.nCopies(23,"0")))&&exp<0){
            exp++;
            sig = rightShift(sig,1);
        }
        if(exp>=255){
            return ""+sign+"11111111"+String.join("",Collections.nCopies(23,"0"));
        }
        if(exp<0){
            return sign=='0'?IEEE754Float.N_ZERO:IEEE754Float.P_ZERO;
        }
        if(exp==0){
            sig = sig.substring(1)+"0";
        }
        return round(sign, Transformer.intToBinary(Integer.toString(exp)).substring(32-8),sig);
    }
...
//        return Qstr;
        String ans= "";
        x= x + String.join("",Collections.nCopies(length,"0"));
        String divisor = oneAdder(negation(y)).substring(1);
        for(int i = 0;i < length;i++){
            String temp = carry_adder(x.substring(0,length),divisor,0,length).substring(1);
            if(temp.charAt(0)=='0'){
                x = temp.substring(1)+x.substring(length)+"1";
            }
            else
